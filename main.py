...
class GetMessagesHandler(BaseHandler):
    def post(self):
        ...
        prevMsgs = self.user.messages
        if prevMsgs and recipient in prevMsgs:
            self.response.write(json.dumps(prevMsgs[recipient], ensure_ascii=True))
        else:
            self.response.write('{}')

class StoreMessageHandler(BaseHandler):
    def post(self):
        ...
        if self.user.messages is None:
            self.user.messages = {}
        if recipient not in self.user.messages:
...
app = webapp2.WSGIApplication([
    ...
    webapp2.Route('/getMsgs', GetMessagesHandler, name='getMsgs'),
    webapp2.Route('/storeMsg', StoreMessageHandler, name='storeMsg'),
